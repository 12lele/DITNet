...
def test(data,
    ...
    if not isinstance(tp, int):
        return (tp[0], fp[0], fn[0], f1[0], mp, mr, map50, map75, map, *(loss.cpu() / len(dataloader)).tolist()), maps, MRresult, t
    else:
        return (tp, fp, fn, f1, mp, mr, map50, map75, map,
                *(loss.cpu() / len(dataloader)).tolist()), maps, MRresult, t


if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='test.py')
    parser.add_argument('--weights', nargs='+', type=str, default='/data1/lxl/ICAFusion/runs/train/83.1/weights/best.pt', help='model.pt path(s)')
    ...
    p = "/data1/FLIR-align-3class/labels/test"
